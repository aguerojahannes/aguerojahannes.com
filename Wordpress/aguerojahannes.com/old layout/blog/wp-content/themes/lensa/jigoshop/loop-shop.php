...
query_posts( array_merge( $wp_query->query, array( 'posts_per_page' => $per_page ) ) );
...
if (have_posts()) : while (have_posts()) : the_post(); $_product = new jigoshop_product( $post->ID ); $loop++;
	...
	?>
	<li class="product gallery-item jigo-product <?php if ($loop%$columns==0) echo 'last'; if (($loop-1)%$columns==0) echo 'first'; ?>">
